...
// Creates a `oauth2.CodeClient` used for authorization (scope) requests.
  ...
  Future<void> _onPromptMoment(
    ...
    String extractReason() {
      return moment.getNotDisplayedReason()?.toString() ??
    ...
    if (momentType != null) {
      switch (momentType) {
        case MomentType.dismissed:
          final MomentDismissedReason? dismissedReason =
              moment.getDismissedReason();
          if (dismissedReason == MomentDismissedReason.credential_returned) {
            // Delay completion so the credential stream emits before we read it.
            return Future<void>.delayed(Duration.zero, () {
              completer.complete(
                  utils.gisResponsesToUserData(_lastCredentialResponse));
            });
          }
          _credentialResponses.addError(extractReason());
          completer.complete(null);
          return;
        case MomentType.skipped:
          _credentialResponses.addError(extractReason());
          completer.complete(null);
          return;
        case MomentType.display:
          final MomentNotDisplayedReason? notDisplayedReason =
              moment.getNotDisplayedReason();
          if (notDisplayedReason != null) {
            _credentialResponses.addError(notDisplayedReason.toString());
            completer.complete(null);
          }
          return;
      }
    ...
    if (moment.isNotDisplayed() ||
      ...
      final String reason = extractReason();
