...
class GoogleSignInPlugin extends GoogleSignInPlatform {
  ...
  GoogleSignInPlugin({
    ...
    if (debugOverrideLoader) {
      ...
      _jsSdkLoadedFuture =
          loader.loadWebSdk().catchError((Object error, StackTrace stackTrace) {
        // In case the loader fires the callback twice (e.g. script already present),
        // fall back to a resolved future so initialization can continue.
        if (error is StateError && error.message == 'Future already completed') {
          if (kDebugMode) {
            debugPrint(
              '[google_sign_in_web] GIS loader completed twice; proceeding with existing script.',
            );
          }
          return;
        }
